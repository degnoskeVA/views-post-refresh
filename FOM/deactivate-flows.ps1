...
sfdx force:source:retrieve -m ApexTrigger:ViewsSSNUpdateTrigger  -u $username
(Get-Content -path ../views-post-refresh/force-app/main/default/triggers/ViewsSSNUpdateTrigger.trigger-meta.xml -Raw) -replace 'Active','Inactive'| Set-Content -Path ../views-post-refresh/force-app/main/default/triggers/ViewsSSNUpdateTrigger.trigger-meta.xml
sfdx force:source:deploy -m ApexTrigger:ViewsSSNUpdateTrigger  -u $username

# Deactivate AccountTrigger
sfdx force:source:retrieve -m ApexTrigger:AccountTrigger  -u $username
...
sfdx force:source:retrieve -m "WorkflowRule:Account.FOM Update External ID"  -u $username
...
sfdx force:source:retrieve -m "WorkflowRule:Account.Update Owner on VA Org Record Type"  -u $username
(Get-Content -path ../views-post-refresh/force-app/main/default/workflows/Account.workflow-meta.xml -Raw) -replace '<active>true</active>','<active>false</active>'| Set-Content -Path ../views-post-refresh/force-app/main/default/workflows/Account.workflow-meta.xml
sfdx force:source:deploy -m "Workflow:Account"  -u $username

# Deactivate CF_Account_Related_Functionality_Edit_Account workflow Rule
sfdx force:source:retrieve -m "flowDefinition:CF_Account_Related_Functionality_Edit_Account"  -u $username
...
sfdx force:source:retrieve -m "flowDefinition:CF_Move_Contact_To_New_Office_Process"  -u $username
...
sfdx force:source:retrieve -m "flowDefinition:CF_Mark_User_As_Inactive"  -u $username
...
sfdx force:source:retrieve -m "flowDefinition:CF_User_When_the_Contact_is_Created_Process"  -u $username
...
sfdx force:source:retrieve -m "flowDefinition:Create_FOM_Data_Collection_Requests"  -u $username
...
sfdx force:source:retrieve -m "flowDefinition:FOM_Default_Account_POC_from_POC_If_Null"  -u $username
...
# Deactivate ViewsSSNUpdateTrigger
sfdx force:source:retrieve -m ValidationRule:Account.FOM_Activities_Field_Required_On_Level2s  -u $username
